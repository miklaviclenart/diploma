\section{Družina \texorpdfstring{\(\lambda e^z\)}{\lambda exp(z)}} \label{sec:druzina}

Dokazali smo, da je \(e^z\) kaotična na celi kompleksni ravnini. Sledi, da je njena Juliajeva množica enaka \(\CC\). Zanimivo pa je, da to ne drži nujno že, če preslikavo pomnožimo s konstanto. V tem poglavju bomo preučili dinamiko \emph{eksponentne družine}
...
\begin{trditev}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \item Če je \(0 < \lambda < 1/e\), ima \(E_\lambda\) dve realni fiksni točki: \(x^* \in (0, 1)\) (privlačna) ter \(x^{**} > 1\) (odbojna).
        \item Če je \(\lambda < 0\), ima \(E_\lambda\) eno realno fiksno točko \(x^*\):
              \begin{itemize}
                  \item \(x^*\) je privlačna za \(-e < \lambda < 0\);
                  \item \(x^*\) je odbojna za \(\lambda < -e\);
                  \item \(x^*\) je nevtralna za \(\lambda = -e\).
              \end{itemize}
    \end{enumerate}
\end{trditev}
...
\begin{lema} \label{lem:enadva}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \item če je \(l_\lambda \in (\ln (1 / \lambda), p_\lambda)\) realno število, je \(E_\lambda (l_\lambda) < l_\lambda\).
    \end{enumerate}
\end{lema}
...
\begin{trditev} \label{prop:trinajst}
    ...
    \begin{enumerate}[label=(\arabic*)]
        ...
        \item Naj bo \(\nu = \ln (1 / \lambda) > 1\) in \(H\) polravnina \(\re z < \nu\). Potem je \(H\) vsebovana v neposrednem območju privlaka \(\Omega_0 (E_\lambda, q_\lambda)\).
...
\begin{dokaz}
    ...
    (2) Naj bo \(z\) vsebovan v polravnini \(\re z < p_\lambda\) in \(l_\lambda \in (\ln (1 / \lambda), p_\lambda) \cap (\re z, p_\lambda)\). Po trditvi \ref{prop:privlak}, uporabljeni na polravnini \(\re \zeta < l_\lambda\), vemo, da vsaka orbita z začetno točko v polravnini \(\re \zeta < l_\lambda\) konvergira k \(q_\lambda\).

    (3) Za \(\re z \geq l_\lambda\) velja
    \[|E_\lambda' (z)| = \lambda e^{\re z} \geq \lambda e^{l_\lambda} = \mu > \lambda e^{\ln (1 / \lambda)} = 1.\]

    (4) Sklepamo podobno kot v točki (2).
    ...
    (6) Če je \(z = x + (2n + 1) \pi i\), je \(E_\lambda (z) = - \lambda e^x \in H \subset \Omega_0 (E_\lambda, q_\lambda)\). Po drugi strani je območje privlaka popolnoma invariantno in zato \(z \in \Omega (E_\lambda, q_\lambda)\). Ker je množica \(H \cup \set{z : \im z = (2n + 1) \pi}\) povezana, je \(\set{z : \im z = (2n + 1) \pi}\) vsebovana v \(\Omega_0 (E_\lambda, q_\lambda)\).
\end{dokaz}
...
\begin{dokaz}
    Posledica sledi po drugi točki trditve \ref{prop:trinajst}.
\end{dokaz}
...
\begin{dokaz}
    Naj bo \(z_0\) periodična točka \(E_\lambda\) s periodo \(p\) in \(z_0, z_1, \dots, z_{p - 1}\) njena orbita. Vsaka točka \(z_j\) ima periodično orbito in zato ni konvergentna. Torej nobena ni vsebovana v polravnini \(\re \zeta < p_\lambda\). Zato \(\re z_j \geq p_\lambda > \ln (1 / \lambda)\). Po tretji točki trditve \ref{prop:trinajst} vemo, da je \(|E_\lambda' (z_j)| > 1\) za \(j = 0, 1, \dots, p - 1\). Po verižnem pravilu izračunamo
    \[\bigl( E_\lambda^p \bigr)' (z_0) = \prod_{j = 0}^{p - 1} E_\lambda' (z_j),\]
...
\begin{enumerate}[label=(\arabic*)]
    \item Velja \(B (N) \subset \Int (A)\).
    \item Funkcija \(E_\lambda\) preslika \(B (N)\) v \(A\).
\end{enumerate}

\noindent Dokažimo najprej lastnost (1). Naj bo \(z = x + i y \in B (N)\). Po definiciji \(B (N)\) imamo
...
Dokažimo še lastnost (2). Naj bo \(z \in B (N)\). Potem \(E_\lambda (z) = \lambda e^x e^{i y}\) in zato
\[|E_\lambda (z)| = \lambda e^x \leq \lambda e^{r_\lambda} \quad \text{in} \quad |E_\lambda (z)| = \lambda e^x \geq \lambda e^{l_\lambda}.\]

Vrnimo se nazaj h konstrukciji Juliajeve množice \(E_\lambda\). Za vsak \(i \in \{ -N, \dots, N \}\) definiramo ``podpravokotnik''
\[R_i \coloneq \bigl\{ x + iy \in \CC : x \in [l_\lambda, r_\lambda], y \in [(2i - 1) \pi, (2i + 1) \pi] \bigr\}.\]
Ker po lastnosti (2) \(E_\lambda\) preslika \(R_i\) v \(A\), ostaja pokazati le, da je ta preslikava surjektivna. Naj bo \(w = r \lambda e^{i \theta} \in A\), torej \(r \in (e^{l_\lambda}, e^{r_\lambda})\) in \(\theta \in [(2i - 1) \pi, (2i + 1) \pi]\). Če vzamemo \(x = \ln r\) in postavimo \(z = x + i \theta\), je po definiciji \(z \in R_i\) in
\[E_\lambda (z) = \lambda e^x e^{i \theta} = \lambda r e^{i \theta} = w.\]
Tako je res \(E_\lambda (R_i) = A\). Ker to drži za vsak \(i \in \{ -N, \dots, N \}\), sledi tudi, da je za poljubna \(i, j\): \(R_j \subset E_\lambda (R_i)\).

Po točki (3) trditve \ref{prop:trinajst} obstaja \(\mu > 1\), da velja
\begin{equation} \label{eqn:mu-ocena}
    \bigl| E_\lambda' (z) \bigr| \geq \mu > 1 \text{ na polravnini } \re z \geq l_\lambda.
\end{equation}
...
to je množico točk, katerih orbite pod \(E_\lambda\) ostanejo v \(B (N)\). Za vsak \(k \in \{ -N, \dots, N \}\) uvedemo trak
\[S (k) \coloneq \set{x + i y \in \CC : (2k - 1) \pi \leq y \leq (2k + 1) \pi}.\]
Naj bo \(L_k\) veja inverza preslikave \(E_\lambda\), definirana na negativno zarezani ravnini \(\CC \setminus (- \infty, 0]\), tako da
\[L_k (z) = \ln \prt{\frac{|z|}{\lambda}} + i \Arg_k (z),\]
kjer je \(\Arg_k (z) \in ((2k - 1) \pi, (2k + 1) \pi)\). Tako ima \(L_k\) vrednosti v \(S (k)\). Potem za vsak \(z \in \CC \setminus (- \infty, 0]\) in \(k \in \{ - N, \dots, N \}\) drži
\begin{equation} \label{eqn:inverz-e}
    E_\lambda \circ L_k (z) = z.
\end{equation}
...
Za neki \(z \in \Lambda_N\) je \(\im (E_\lambda^n (z)) \neq (2j \pm 1) \pi\) za vsak \(n \in \NN\) in vsak \(j \in \{ -N, \dots, N \}\). Res, če je \(E_\lambda^n (z) = x + (2j \pm 1) \pi i\), je \(E_\lambda^{n + 1} (z) = - \lambda e^x \notin B (N)\). Zato vsaka točka \(E_\lambda^n (z)\) leži v pravokotniku
\[R_i^* \coloneq \bigl\{ x + iy \in \CC : x \in [l_\lambda, r_\lambda], y \in ((2i - 1) \pi, (2i + 1) \pi) \bigr\},\]
...
vsebuje natanko eno točko, ki jo označimo z \(z_{\overline{s}}\). Ker po trditvi \ref{prop:claim} za vsaka \(j, k\) velja
...
\begin{dokaz}
    Začnimo z dokazom surjektivnosti. Naj bo \(\overline{s} \in \Sigma_N\) itinerar točke \(z \in \Lambda_N\). Z indukcijo pokažemo, da je \(\phi (\overline{s}) = z\). Za \(n = 0\) po definiciji \(z \in B (N)\) velja \(L_{s_0} \circ E_\lambda (z) = z\). Predpostavimo
    ...
    Preverimo, da je \(\phi\) zvezna. Naj bo \(\overline{s}^o \in \Sigma_N\) in \(z_0 = \phi (\overline{s}^o)\). Za \(\varepsilon > 0\) izberemo tak \(k\), da
    ...
    Tako je \(\phi\) zvezna. Ker je \(\Sigma_N\) kompakten in \(\phi\) bijektivna, pa je zvezna tudi \(\phi^{-1}\).

    Nazadnje premislimo, da \(\phi\) podaja ustrezno konjugacijo obeh dinamičnih sistemov. Če je \((s_0, s_1, s_2, \dots) = \phi^{-1} (z)\), je \(z \in L_s^{[n]} \prt{B (N)}\) za vsak \(n \in \NN\) in zato \(E_\lambda (z) \in E_\lambda \circ L_s^{[n]} \prt{B (N)}\) za vsak \(n \in \NN\). Uporabimo \eqref{eqn:inverz-e}, da dobimo \(E_\lambda (z) \in L_s^{[n]} \prt{B (N)}\) za vsak \(n \in \NN\). To pomeni
    \[E_\lambda (z) = \phi (s_1, s_2, \dots) = \phi \circ \sigma (s_0, s_1, s_2, \dots) = \phi \circ \sigma \circ \phi^{-1} (z).\]
\end{dokaz}

\noindent Končno lahko dokažemo enakost
\[\overline{\bigcup_{n \geq 1} \Lambda_N} = J (E_\lambda).\]
Po neenakosti \(|E_\lambda' (z)| \geq \mu > 1\) \eqref{eqn:mu-ocena} za vsak \(z \in \Lambda_N\) in izreku \ref{thm:julm}, velja \(\Lambda_N \subset J (E_\lambda)\). Ker je \(J (E_\lambda)\) po izreku \ref{thm:closure} zaprtje množice vseh odbojnih periodičnih točk, je dovolj, da dokažemo, da je vsaka taka točka v kakšni od množic \(\Lambda_N\). Naj bo \(z_0\) odbojna periodična točka preslikave \(E_\lambda\). S posledico \ref{cor:jul-e} vemo, da \(z_0 \in \{\re z \geq l_\lambda\}\). Periodičen cikel \(\{z_0, E_\lambda (z_0), \dots\}\) je tako omejen in leži na polravnini \(\{\re z \geq l_\lambda\}\), zato je za dovolj velik \(N\) v celoti vsebovan v \(B (N)\). Po definiciji \(\Lambda_N\) so vse točke cikla v \(\Lambda_N\), torej je tudi \(z_0 \in \Lambda_N\).
