...
Pokazali bomo, da kompleksna eksponentna preslikava \(f\) ustreza definiciji topološke tranzitivnosti. Iz tega nato pokažemo močnejšo trditev, da je množica točk z gosto orbito gosta.

\begin{izrek} \label{thm:transitivity}
    Kompleksna eksponentna preslikava je topološko tranzitivna.
\end{izrek}

\noindent Pomagamo si z naslednjimi pomožnimi rezultati.

\begin{trditev}[Raztezanje vzdolž ubežnih orbit]
    Naj bo \(z_0 \in I (f)\) in \(z_n = f^n (z_0)\) za \(n \in \NN\). Potem je
    \[\lim_{n \to \infty} \re z_n = \infty \quad \text{in} \quad \lim_{n \to \infty} |(f^n)' (z_0)| = \infty.\]
\end{trditev}
...
\begin{dokaz}
    ...
    \[\abs{\prt{f^{m}}' (z_0)} = \abs{ \prt{f^{N}}' (z_0) } \cdot \prod_{n = N}^{m - 1} \abs{f' (z_n)} \geq 2^{m - N} \cdot \abs{ \prt{f^{N}}' (z_0) }.\]
    Za vsak \(z \in \CC\) velja \(f' (z) \neq 0\) in zato \(|(f^N)' (z_0)| \neq 0\). Zato sklepamo, da je
    \[\lim_{m \to \infty} |(f^{m})' (z_0)| = \infty.\]
\end{dokaz}

\noindent Spomnimo, da \emph{premer} množice \(A\) definiramo kot \(\diam (A) \coloneq \sup \set{|x - y| : x, y \in A}\).

\begin{trditev}[Raztezanje malih diskov] \label{prop:small_disks}
    Naj bo \(z_0 \in I (f)\) in \(z_n = f^{n} (z_0)\). Za diske \(D_n \coloneq \Delta (z_n, 2 \pi)\) in neki \(n_0 \in \NN\) obstaja zaporedje \(\left\{ \phi_n \right\}_{n = n_0}^{\infty}\) holomorfnih preslikav \(\phi_n \colon D_n \to \CC\) z naslednjimi lastnostmi:
    \begin{enumerate}
        ...
        \item \label{item:stiri} \(\lim_{n \to \infty} \diam (\phi_n (D_n)) = 0\).
    \end{enumerate}
\end{trditev}

\begin{dokaz}
    Naj bo \(z \in D_n\). Potem po integralski oceni (glej \cite[opomba na str.~233]{Globevnik_2010}) velja
    \begin{equation}
        |\phi_n (z) - \phi_n (z_n)| \leq |z - z_n| \cdot \sup_{\zeta \in D_n} |\phi_n' (\zeta)| \leq 2 \pi \sup_{\zeta \in D_n} |\phi_n' (\zeta)|.
    \end{equation}
    ...
    Spet uporabimo integralsko oceno od prej in vidimo, da za vsak \(n\) in vsak \(z \in D_n\) velja
    \[\absolute{L_n (z) - z_{n - 1}} = \absolute{L_n (z) - L_n (z_n)} \leq 2 \pi \cdot \sup_{\zeta \in D_n} \absolute{L_n' (\zeta)} \leq \pi.\]
    To pomeni, da imamo \(L_n (D_n) \subset D_{n - 1}\). Z indukcijo sklepamo, da je kompozitum
    \[\phi_n \coloneq L_1 \circ L_2 \circ \dots \circ L_n\]
    definiran na \(D_n\) in za vsak \(z \in D_n\) velja \(\absolute{\phi_n' (z)} \leq 2^{-n}\). Torej za preslikavo \(\phi_n\) velja točka \ref{item:tri}, točki \ref{item:ena} in \ref{item:dva} pa veljata po njeni definiciji.

    Če predpostavka \(\absolute{z_n} \geq 2 \pi + 2\) ne velja, sklepamo na sledeč način. Ker je \(z_0\) ubežna točka, še vedno obstaja tak \(n_1 \in \NN\), da za vsak \(n > n_1\) velja \(\absolute{z_n} \geq 2 \pi + 2\). To pa pomeni, da za vsak tak \(n\) obstaja holomorfna preslikava \(\psi_n \colon D_n \to D_{n_1}\), za katero veljajo vse točke trditve tako, da za \(z_0\) v trditvi izberemo \(z_{n_1}\) z lastnostmi:
    \begin{enumerate}
        \item \(\psi_n (z_n) = z_{n_1}\);
        \item za vsak \(z \in D_n\) velja \(f^{n - n_1} (\psi_n (z)) = z\);
        \item \(\lim_{n \to \infty} \sup_{z \in D_n} |\psi_n' (z)| = 0\);
        \item \(\lim_{n \to \infty} \diam (\psi_n (D_n)) = 0\).
    \end{enumerate}
    ...
    Naj bo \(K\) majhen zaprt disk okrog \(z_{n_1}\), tako da je \(K \subset U\). Po točki \ref{item:stiri} za dovolj velik \(n\), na primer \(n \geq n_0\), velja \(\psi_n (D_n) \subset K\). Tako lahko definiramo
    ...
    Ker je \(K\) kompaktna, zvezna funkcija \(\absolute{\pi'}\) na njej zavzame svoj maksimum, ki je neodvisen od \(n\). Zato je
...
\begin{trditev}[Slike velikih diskov] \label{prop:large_disks}
    Naj bo \(K\) neprazna kompaktna podmnožica \(\CC \setminus \set{0}\). Potem obstaja \(\rho > 0\), da za vsak disk \(D\) radija \(2 \pi\) s središčem v \(\zeta \in [\rho, \infty) \times \CC\) velja \(K \subset f^2 (D)\).
\end{trditev}

\noindent Trditev nam pove, da že druga iteracija diska, ki leži ``dovolj'' na desno, pokrije velik del kompleksne ravnine.

\begin{dokaz}
    Disk \(D\) vsebuje zaprt kvadrat \(S\) s stranico dolžine \(2 \pi\), prav tako s središčem v \(\zeta\). Naj bo \(a = \re (\zeta) - \pi\) realni del leve navpične stranice \(S\). Potem je \(a + 2 \pi = \re (\zeta) + \pi\) realni del desne navpične stranice \(S\). Preslikava \(f\) kvadrat \(S\) preslika v krožni kolobar \(A\) s središčem v izhodišču, notranjim radijem \(r_{-} = e^a\) in zunanjim radijem \(r_{+} = e^{a + 2 \pi}\) (glej sliko \ref{fig:exponential}). Če sta \(\rho\) in posledično \(a\) dovolj velika, je kolobar \(A\) dovolj širok, da vsebuje pravokotnik višine \(2 \pi\). Slika tega pravokotnika nato pokrije večino kompleksne ravnine, vključno s kompaktno množico \(K\). Bolj natančno; naj bo \(a \geq 0\) in zato \(r_+ - r_- \geq e^{2 \pi} > 2 \pi\). Torej \(A\) vsebuje pravokotnik \(R\) višine \(2 \pi\), tako kot je prikazano na sliki \ref{fig:transitivity}.

    \begin{figure}[hb]
        \centering
        \input{tikz/transitivity.tex}
        \caption{Ilustracija dokaza trditve \ref{prop:large_disks}.}
        \label{fig:transitivity}
    \end{figure}

    \noindent Naj bo \(l\) polovica daljše stranice pravokotnika \(R\), ki jo izračunamo po Pitagorovem izreku
    ...
    \[\rho \coloneq 3 \pi + \max_{z \in K} \, \bigl| \log \abs{z} \bigr|.\]
    Če je \(\re \zeta \geq \rho\), je \(a \geq 2 \pi\) in \(l > e^a > a > \abs{\log \abs{z}}\).
\end{dokaz}
...
\begin{posledica}[Odprte množice se razširijo vsepovsod]
    Naj bo \(K \subset \CC \setminus \set{0}\) kompaktna, \(U \subset \CC\) pa odprta in neprazna. Potem obstaja \(N \in \NN\), da je \(K \subset f^n (U)\) za vse \(n \geq N\).
\end{posledica}

\noindent \textit{Opomba.} Iz posledice takoj dokažemo izrek \ref{thm:transitivity}, če za \(K\) izberemo točko iz \(V\).

\begin{dokaz}
    Ker so ubežne točke goste, obstaja \(z_0 \in I (f) \cap U\). Naj bo \(z_n \coloneq f^n (z_0)\) za \(n \geq 1\) in naj bodo \(D_n \coloneq \Delta (z_n, 2 \pi)\), \(n_0\) in \(\phi_n\) kot v trditvi \ref{prop:small_disks}. Naj bosta \(\rho\) in \(K\) kot v trditvi \ref{prop:large_disks}. Ker je \(z_0\) ubežna točka, po točki \ref{item:stiri} trditve \ref{prop:small_disks} obstaja \(N_1 \geq n_0\), da je \(\re z_n \geq \rho\) in \(\phi_n (D_n) \subset U\) za vsak \(n \geq N_1\). Naj bo sedaj \(n \geq N_1\). Potem po trditvi \ref{prop:large_disks} velja
...
\begin{dokaz}
    Naj bo \(D \subset \CC\) neprazna odprta množica. Po izreku \ref{thm:transitivity} je inverzna orbita
    ...
    Ker je \(\TTT\) števen presek odprtih gostih podmnožic \(\CC\), je po Bairovem izreku (glej \cite[izrek 5.6]{Rudin_1987}) neštevna in gosta v \(\CC\).
\end{dokaz}