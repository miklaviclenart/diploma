...
Neprazni, odprti in povezani podmnožici kompleksnih števil \(D \subseteq \CC\) pravimo \emph{območje}. Večinoma se bomo ukvarjali s kompleksnimi funkcijami ene spremenljivke \(f \colon D \to \CC\). Le-te lahko zapišemo kot vsoto realnega in imaginarnega dela \(f = u + iv\), kjer sta za \(z = x + i y\) \(u = u (x, y)\) in \(v = v (x, y)\) ustrezni realni funkciji. Naj bo od tu naprej \(U \subseteq \CC\) odprta množica.
...
Holomorfnost razumemo kot odvedljivost v kompleksnem smislu. Zato zgornji limiti pravimo \emph{odvod funkcije \(f\) v točki \(z\)} in jo označimo z \(f' (z)\). Kot za realne veljajo tudi za kompleksne funkcije običajna pravila za odvajanje. Poleg tega pa nam holomorfnost karakterizira tudi naslednji izrek.

\begin{izrek} \label{thm:cr}
    Funkcija \(f = u + i v\) je holomorfna natanko tedaj, ko funkciji \(u\) in \(v\) zadoščata \emph{Cauchy-Riemannovemu sistemu enačb}
    \begin{equation} \tag{CR} \label{eq:cr}
        \frac{\partial u}{\partial x} = \frac{\partial v}{\partial y}, \qquad \frac{\partial u}{\partial y} = - \frac{\partial v}{\partial x}.
    \end{equation}
    Natančneje, funkcija \(f\) je na množici \(U\) holomorfna natanko tedaj, ko sta \(u\) in \(v\) na množici \(\set{(x, y) \in \RR^2 : x + i y \in U}\) zvezno parcialno odvedljivi in zadoščata sistemu \eqref{eq:cr}.
\end{izrek}

\begin{dokaz}
    Označimo \(f = u + i v\), \(z_0 = x + i y\) in \(\zeta = h + i k\). Potem je
    \[\frac{f (z + \zeta) - f (z)}{\zeta} = \frac{u (x + h, y + k) - u (x, y)}{h + i k} + i \frac{v (x + h, y + k) - v (x, y)}{h + i k}.\]
    Ker za \(f (z) \neq 0\) lahko obravnavamo \(\widetilde{f} \coloneq f - f (z)\), lahko brez škode za splošnost predpostavimo \(f (z) = 0\). Tedaj se zgornja enačba poenostavi v
    \[\frac{f (z + \zeta)}{\zeta} = \frac{u (x + h, y + k)}{h + i k} + i \frac{v (x + h, y + k)}{h + i k}.\]
    Krajše pišemo \(\)nožimo z \((h - i k)\)
\end{dokaz}
...
Funkcija \(f \colon U \to U'\) je \emph{biholomorfna}, če je bijektivna, holomorfna in ima holomorfen inverz \(f^{-1} \colon U' \to U\). Če je \(D\) območje in \(f \colon D \to D'\) bijektivna in holomorfna, vemo, da je biholomorfna (glej \cite[izrek 77]{Globevnik_2010}). \emph{Enostavno povezano} območje si intuitivno predstavljamo kot tisto območje, ki nima ``lukenj''. Natančno pa ga definiramo definiramo z naslednjim izrekom.
...
je tudi periodičen in posledično nima inverza. Res, naj bo \(w = u + i v\). Rešimo enačbo:
\begin{align*}
...
\begin{itemize}
    ...
    \item Vodoravne premice se preslikajo v žarke iz izhodišča, v posebnem se realna os preslika v \([0, \infty)\). Navpične črte se preslikajo v krožnice okrog izhodišča, v posebnem se imaginarna os preslika v enotsko križnico. Splošna premica se preslika v spiralo, ki se navija okrog izhodišča.
    \item Sivo obarvana leva polravnina se preslika v enotski disk, desna polravnina pa v njegov komplement.
    \item Slike majhnih kvadratkov nas spominjajo na kvadratke, kar ponazarja, da je preslikava konformna---ohranja kote.
\end{itemize}
