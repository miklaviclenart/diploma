\section{Kaotičnost kompleksnega eksponentna} \label{sec:dokaz}

Od tu naprej naj velja, da \(f\) vedno označuje kompleksno eksponentno preslikavo
\[f \colon \CC \to \CC; \qquad z \mapsto e^{z}.\]
V tem poglavju dokažemo izrek \ref{thm:orbits}, v vsakem od podpoglavij po eno točko. Dokaz je povzet po članku \cite{Shen_2015}.

\subsection{Gostost ubežnih točk}

Točka \(z \in \CC\) je \emph{ubežna}, če njena orbita konvergira k \(\infty\). Množico vseh ubežnih točk \(I (f)\) imenujemo \emph{ubežna množica}, torej
\[I (f) \coloneq \set{z \in \CC : \lim_{n \to \infty} \abs{f^n (z)} = \infty}.\]

\begin{izrek} \label{thm:escapingdense}
    Ubežna množica kompleksne eksponentne preslikave je gosta podmnožica kompleksne ravnine.
\end{izrek}
...
\begin{lema}[Eksponentna preslikava veča hiperbolično metriko] \label{lem:hyper}
    ...
    \[\lim_{n \to \infty} \min \bigl\{ |\zeta_n|, \Arg (\zeta_n) \bigr\} = 0.\]
\end{lema}
...
\begin{dokaz}
    ...
    \]
    Da poenostavimo imenovalec, opazimo, da je \(e^{\zeta} = e^{r (\cos \theta + i \sin \theta)}\) periodična s periodo \(2 \pi\). Zato velja \(\arg (e^{\zeta}) \equiv r \cdot \sin (\theta) \pmod{2 \pi}\). Dodatno je še \(|\sin|\) periodična s periodo \(\pi\) in zato velja \(|\sin (\arg (e^{\zeta}) / 2)| = |\sin (r \cdot \sin (\theta) / 2)|\). Posledično je
    ...
    \[\hder{f (\zeta_n)}{U}{U} \geq \frac{1}{|\cos (\theta_n / 2)|} > 1.\]
    Uporabimo predpostavke leme, iz česar sledi
    \[\lim_{n \to \infty} \frac{1}{|\cos (\theta_n / 2)|} = 1\]
    in zato \(\lim_{n \to \infty} \Arg (\zeta_n) = 0\).
\end{dokaz}
...
\begin{dokaz}[Dokaz izreka \ref{thm:escapingdense}]
    Naj bo \(w_0 \in \CC\) in \(w_n = f^n (w_0)\) za \(n \in \NN\). Naj bo \(D \coloneq \Delta (w_0, r)\). Dokazujemo s protislovjem, zato predpostavimo, da za vsak \(n \in \NN\) velja \(f^n (D) \cap I (f) = \emptyset\). Vemo, da je \([0, \infty) \subseteq I (f)\), in zato
    ...
    Označimo \(\eta_n \coloneq \hder{f (w_n)}{U}{U}\) in s pomočjo verižnega pravila opazimo \(\delta_{n + 1} = \delta_n \cdot \eta_n\) za vsak \(n \geq 0\). Po lemi \ref{lem:hyper} vemo, da je \(\eta_n > 1\). Zato je zaporedje \(\delta_n\) strogo naraščajoče in navzgor omejeno z \(\num{1}\). Torej lahko izračunamo
    \[\lim_{n \to \infty} \eta_n = \lim_{n \to \infty} \frac{\delta_{n + 1}}{\delta_n} = \frac{\lim_{n \to \infty} \delta_{n + 1}}{\lim_{n \to \infty} \delta_n} = 1.\]
    Ker veljajo predpostavke, po lemi \ref{lem:hyper} vemo
    \begin{equation} \label{eqn:eta1}
        \lim_{n \to \infty} \min \bigl\{ |w_n|, \Arg (w_n) \bigr\} = 0.
    \end{equation}
    Zato morajo vsa stekališča zaporedja \(\zap{w}\) ležati na intervalu \([0, \infty)\). Ker \(w_0\) po predpostavki ni ubežna, množica stekališč ni prazna. Ker je tudi zaprta, ima minimum, ki ga označimo z \(z_0 \in [0, \infty)\). Vemo, da je število \(z_0\) stekališče zaporedja \(w_n\) natanko tedaj, ko obstaja podzaporedje, ki konvergira k \(z_0\) (glej \cite[izrek 11]{Globevnik_2012}). To podzaporedje označimo z \(w_{n_{k}}\). Opazimo, da velja
    \[\re w_{n_{k - 1}} = \log \, \bigl| w_{n_{k}} \bigr| \quad \text{in} \quad \lim_{k \to \infty} \log \, \bigl| w_{n_{k}} \bigr| = \log z_0 < z_0.\]
    Ker je \(z_0\) najmanjše stekališče zaporedja \(w_n\), podzaporedje \(w_{n_{k - 1}}\) ne more imeti končnega stekališča in zato je
    \[\lim_{k \to \infty} \, \bigl|\im w_{n_{k-1}}\bigr| = \infty.\]
    To je v protislovju z \eqref{eqn:eta1}.
\end{dokaz}
