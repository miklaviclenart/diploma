...
\begin{definicija}
    Naj bo \(X\) metrični prostor, \(f \colon X \to X\) preslikava in \(x^*\) fiksna točka \(f\). Če obstaja tak \(r > 0\), da na odprti krogli \(B (x^*, r)\) zaporedje iteracij \(f\) enakomerno konvergira proti \(x^*\), je \(x^*\) \emph{privlačna}. Točka \(x^*\) je \emph{odbojna}, če obstaja \(r > 0\), da za vsak \(x_0 \in B (x^*, r)\) in \(x_0 \neq x^*\), obstaja \(n \in \NN\), za katerega je \(f^n (x_0) \notin B (x^*, r)\).
\end{definicija}

\begin{definicija}
    Naj bosta \(X\) in \(Y\) metrična prostora ter \(f \colon X \to X\) in \(g \colon Y \to Y\) preslikavi. Pravimo, da sta \(f\) in \(g\) \emph{konjugirani}, če obstaja homeomorfizem \(h \colon X \to Y\), da velja \(h \circ f = g \circ h\). Tedaj pravimo, da \(h\) konjugira \(f\) in \(g\).
\end{definicija}
...
\begin{dokaz}
    Naj bo \(x^*\) fiksna točka \(f\). Tedaj \(g (y^*) = g (h (x^*)) = h (f (x^*)) = h (x^*) = y^*\). Če je \(x^*\) privlačna, tedaj obstaja \(r > 0\), da zaporedje \(\{f^n\}_{n=1}^\infty\) enakomerno konvergira na \(B (x^*, r)\). Ker je \(h^{-1}\) zvezna, obstaja \(s > 0\), tako da za \(y \in B (y^*, s)\) velja \(h^{-1} (y) \in B (x^*, r)\). To pomeni, da zaporedje \(\{f^n \circ h^{-1}\}_{n=1}^\infty\) enakomerno konvergira k \(x^*\) na \(B (y^*, s)\). Ker je \(h\) zvezna, pa velja
...
Na intuitivni ravni si besedo kaos navadno predstavljamo kot nered ali zmedo. V naravoslovju ima ta pojem več ne-ekvivalentnih definicij. Mi se bomo naslonili na definicijo ameriškega matematika Roberta L.~Devaneyja iz leta \num{1986} \cite{Devaney_1986}, v kateri je kaos opredeljen prek periodičnih točk, topološke tranzitivnosti in občutljivosti na začetne pogoje.

\begin{definicija}[Topološka tranzitivnost]
    Naj bo \(X\) metrični prostor in \(f \colon X \to X\) preslikava. Pravimo, da je \(f\) \emph{topološko tranzitivna}, če za vsaki neprazni odprti množici \(U\) in \(V\) obstajata \(z \in U\) in \(n \in \NN \cup \set{0}\), da je \(f^n (z) \in V\).
\end{definicija}
...
Občutljivost na začetne pogoje pomeni, da majhna motnja v začetnih pogojih lahko privede do znatnih sprememb pri iteraciji dinamičnega sistema. Natančno jo podamo z naslednjo definicijo.

\begin{definicija}[Občutljivost na začetne pogoje]
    Naj bo \((X, d)\) metrični prostor in \(f \colon X \to X\) preslikava. Pravimo, da je \(f\) občutljiva na začetne pogoje, če obstaja \emph{občutljivostna konstanta} \(\delta > 0\), da za vsak \(x \in X\) in vsak \(\varepsilon > 0\) obstaja \(y \in X\), da je \(d (x, y) < \varepsilon\) in \(d (f^N (x), f^N (y)) \geq \delta\) za neki \(N \in \NN\).
\end{definicija}

\begin{definicija}[Devaneyjev kaos]
    Naj bo \(X\) metrični prostor. Pravimo, da je preslikava \(f \colon X \to X\) \emph{kaotična}, če so izpolnjeni naslednji pogoji.
    \begin{enumerate}
        \item Množica periodičnih točk je gosta v množici \(X\).
        \item Preslikava \(f\) je topološko tranzitivna.
        \item Preslikava \(f\) je občutljiva na začetne pogoje.
    \end{enumerate}
\end{definicija}
...
\begin{zgled}[Podvojitvena preslikava] \label{ex:double}
    ...
    Za dokaz občutljivosti na začetne pogoje pokažemo, da poljubno blizu katerekoli točke \(x \in [0, 1]\) obstaja \(\tilde{x}\), za katerega za nek \(m \in \NN\) velja \(|f^m (x) - f^m (\tilde{x})| = \frac{1}{2}\). Spet si pomagamo z dvojiškim zapisom. Za \(x = 0{,}d_1 d_2 \dots_{(2)}\) vzamemo
    \[\tilde{x} = 0{,}d_1 d_2 \dots d_N \tilde{d}_{N + 1} d_{N + 2} \dots_{(2)},\]
    kjer je \(\tilde{d}_{N + 1} \neq d_{N + 1}\). Potem je \(|x - \tilde{x}| < 2^{- N}\) in \(|f^N (x) - f^N (\tilde{x})| = \frac{1}{2}\).
\end{zgled}

\noindent Izkaže se, da občutljivosti na začetne pogoje pri definiciji kaotične preslikave v večini primerov ni potrebno zahtevati. Leta \num{1992} so namreč dokazali naslednji izrek \cite{Banks_1992}.

\begin{izrek}
    Naj bo \((X, d)\) neskončen metrični prostor in \(f \colon X \to X\) zvezna preslikava. Če je \(f\) topološko tranzitivna in je množica periodičnih točk \(f\) gosta v \(X\), potem je \(f\) občutljiva na začetne pogoje.
\end{izrek}
...
\begin{dokaz}
    ...
    \begin{align}
        d \prt{f^N (p), f^N (y)} & = d \prt{p, f^N (y)} \nonumber                                                              \\
                                 & \geq d \prt{x, f^{N - k} (q)} - d (x, p) - d \prt{f^N (y), f^{N - k} (q)} \label{eqn:ocena} \\
...
\noindent Zgornji izrek je presenetljiv, saj je nenavadno, da topološke lastnosti implicirajo metrične. Da pa gostost periodičnih točk ne implicira topološke tranzitivnosti ali obratno, nam prikaže naslednji zgled.
...
Simbolična dinamika je orodje, ki nam pomaga pri preučevanju dinamičnih sistemov. Z njo smo se srečali že v zgledu \ref{ex:double}, kjer je bila ključna za dokaz topološke tranzitivnosti in občutljivosti na začetne pogoje.

Naj bo \(N \geq 2\) naravno število. S \(\Sigma_N\) označimo množico neskončnih zaporedij \(\overline{s} = (s_0, s_1, s_2, \dots)\), definirano kot
...
%     Če \(s_j \neq t_j\) za neki \(j \leq n\), mora veljati
%     \[d(\overline{s}, \overline{t}) \geq \frac{1}{2^j} \geq \frac{1}{N^n}.\]
% \end{dokaz}
%
\noindent Z naslednjo lemo pokažemo, da za vsak \(\overline{s} \in \Sigma_N\) in \(k \geq 0\) množice
\[V(\overline{s}, k) \coloneq \big\{ \overline{t} = (t_i) \in \Sigma_N : t_i = s_i, i = 0, 1, \dots, k \big\}\]
tvorijo bazo okolic \(\overline{s}\) za topologijo porojeno z zgornjo metriko.
...
Izkaže se, da je \(\sigma\) zvezna. Res, naj bosta \(\overline{s}, \overline{t} \in \Sigma_N\), \(\varepsilon > 0\) in \(k \in \NN\) tak, da je \(2 / N^{k - 1} (N - 1) < \varepsilon\). Vzamemo \(\delta = 1 / N^{k + 1}\). Če je \(d(\overline{s}, \overline{t}) < \delta\), mora po lemi \ref{lem:seq-metric} veljati \(s_i = t_i\) za vsak \(i \leq k + 1\). To pomeni, da se \(\overline{s}\) in \(\overline{t}\) ujemata na prvih \((k + 1)\) mestih. Torej se \(\sigma (\overline{s})\) in \(\sigma (\overline{t})\) ujemata na prvih \(k\) mestih in spet po istem lemi velja \(d (\sigma (\overline{s}), \sigma (\overline{t})) < \varepsilon\).
...
\begin{dokaz}
    ...
    \[X_1 \coloneq \bigl\{ \overline{t} \in \Sigma_N : t_i = s_i \bigr\} \qquad \text{in} \qquad X_2 = \bigl\{\overline{t} \in \Sigma_N : t_i \neq s_i\bigr\}\]
    in opazimo, da je \((X_1 \cap X) \cup (X_2 \cap X) = X\), \(X_1 \cap X_2 = \emptyset\), \(\overline{s} \in X_1\) in \(\overline{s}^* \in X_2\). Do protislovja bomo torej prišli, če pokažemo, da sta \(X_1\) in \(X_2\) odprti. Ker je
    \[X_2 = \bigcup_{s \neq s_i} \bigl\{ \overline{t} \in \Sigma_N : t_i = s \bigr\},\]
